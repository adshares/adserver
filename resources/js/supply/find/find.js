...
let encodeZones = function (zone_data) {
    let VALUE_GLUE = "\t";
    let PROP_GLUE = "\r";
    let ZONE_GLUE = "\n";
    let fields = {};
    ...
    for (let i = 0; i < zone_data.length; i++) {
        let zone = zone_data[i];
        let entry = [];
        for (let prop in zone) {
...
let logInsertedElement = function(el) {
...
let replaceTag = function (oldTag, newTag, banner) {
    for (let i = 0; i < oldTag.attributes.length; i++) {
        let name = oldTag.attributes[i].name;
...
let prepareElement = function (context, banner, element, contextParam) {
    ...
    if (element.tagName === 'IFRAME') {
        ...
        addListener(window, 'message', function (event) {
            if (event.source === element.contentWindow && event.data) {
                let data, isString = typeof event.data == "string";
                if (isString) {
                ...
                if (data.dwmthLoad) {
                    ...
                    let msg = {dwmthLoad: 1, data: context};
...
let prepareInfoBox = function (context, banner, contextParam) {
    ...
    link.innerHTML = '<svg style="width: 16px; height: 16px; display: block;" viewBox="0 0 15 15" xmlns="http://www.w3.org/2000/svg">' +
...
function isRendered(domObj) {
    ...
    while (domObj !== document.body) {
        ...
        } else return !(domObj.currentStyle
...
function rectIntersect(a, b)
{
    ...
    let num1 = Math.min(a.left + a.width, b.left + b.width);
    let y = Math.max(a.top, b.top);
    let num2 = Math.min(a.top + a.height, b.top + b.height);
    if (num1 >= x && num2 >= y)
...
let aduserPixel = function (impressionId, onload) {
    if (!serverOrigin) return;
    let path = '/supply/register?iid=';
    let url = serverOrigin + path + impressionId;

    if(dwmthURLS[url]) return false;

    let iframe = createIframeFromUrl(url);
...
let createIframeFromUrl = function createIframeFromUrl(url, doc) {
    let iframe = (doc || document).createElement('iframe');
...
let getPageKeywords = function (doc) {
    ...
    if (metaKeywords.content) {
        let tmp = metaKeywords.content.replace(/[\t\r\n]/g, ',').split(',');
        let n = Math.min(MAX_KEYWORDS, tmp.length);
        let tmp2 = [];
        for (let i = 0; i < n; i++) {
...
let findBackfillCode = function(container) {
    let tag = container.querySelectorAll('[type="app/backfill"]')[0];
    let text = null;
    if(tag) {
        text = tag.textContent;
    } else {
        for (let i = 0, n = container.childNodes.length; i < n; i++) {
...
let parseZoneOptions = function(str) {
    let opts = {};
...
let getActiveZones = function(call_func, retryNo) {
    let _tags = document.querySelectorAll(selectorClass + '[data-zone]');
    let n = _tags.length;
    ...
    tags.forEach(function(tag, i) {
        ...
        for (let j = 0, m = tag.attributes.length; j < m; j++) {
            let parts = tag.attributes[j].name.split('-');
            let isData = (parts.shift() === "data");
    ...
    fn = function(){
        if(waiting > 0) {
            ...
            let filter = function(x) { return !x.__invalid; };
...
domReady(function () {
    aduserPixel(getImpressionId(), function () {
        getActiveZones(function (zones, params) {
            let data = encodeZones(params);

            let url = serverOrigin + '/supply/find';
            let options = {
                json: true
            };
            if (data.length <= 800) {
            ...
            addListener(topwin, 'message', function (event) {
                let has_access = dwmthACL.some(function (win) {
                    try {
                    ...
                    if (data.insertElem) {
                        data.insertElem.forEach(function (request) {
                            ...
                            if (request.type === 'iframe') {
                                ...
                                let iframe = addAnalyticsIframe(request.url);
                                dwmthACL.push(iframe.contentWindow);
...
let addAnalyticsIframe = function (url) {
    if (!url) return;
    let iframe = createIframeFromUrl(url, topdoc);
...
let addAnalyticsImage = function (url) {
    if (!url) return;
    let img = new Image();
...
let mapInt = function(hex, min)
{
    let short = hex.substr(0, 8);
    if(short.charAt(0) > '7') {
        short = '7' + short.substr(1);
    }
    let r = parseInt(short, 16);
    if(min && r < min) {
        r += 1*min;
    }
    return r;
...
let fillPlaceholders = function(url, caseId, bannerId, publisherId, serverId, siteId, zoneId, keywords)
{
...
let getDomain = function(url)
{
    ...
    a.href = url;
    let host = a.host.indexOf('www.') === 0 ? a.host.substr(4) :a.host;
    let colonPos = host.indexOf(':');
...
let allBannersLoaded = function()
{
    selectPopCandidate();
}

let fetchBanner = function (banner, context, zone_options) {
    fetchURL(banner.serve_url, {
        ...
        let contextParam = encodeZones([context.page]);
        ...
        let sendViewEvent = function(element) {
