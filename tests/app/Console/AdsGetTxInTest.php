...
class AdsGetTxInTest extends TestCase
{
    ...
    public function testAdsTxIn(): void
    {
        $expectedTxAddedCount = 12;
        $logResponse = $this->getLog();

        $this->app->bind(
            AdsClient::class,
            function () use ($logResponse) {
                $adsClient = $this->createMock(AdsClient::class);
                $adsClient->method('getLog')->willReturn(new GetLogResponse(json_decode($logResponse, true)));
        ...
        $this->artisan('ads:get-tx-in')
            ->expectsOutput(sprintf('Number of added txs: %d', $expectedTxAddedCount))
            ->assertExitCode(0);
        ...
        $this->assertEquals($expectedTxAddedCount, AdsPayment::all()->count());
        $this->assertEquals($expectedTxAddedCount, AdsPayment::where('status', AdsPayment::STATUS_NEW)->count());
    }
