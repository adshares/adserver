...
class WalletWithdrawalCheckCommandTest extends ConsoleTestCase
{
    ...
    public function testAutoWithdrawal(): void
    {
        ...
        factory(UserLedgerEntry::class)->create(
            ['user_id' => $user1->id, 'type' => UserLedgerEntry::TYPE_AD_INCOME, 'amount' => 1000]
        );
        factory(UserLedgerEntry::class)->create(
            ['user_id' => $user2->id, 'type' => UserLedgerEntry::TYPE_AD_INCOME, 'amount' => 1000]
        );
        factory(UserLedgerEntry::class)->create(
            ['user_id' => $user2->id, 'type' => UserLedgerEntry::TYPE_BONUS_INCOME, 'amount' => 100]
        );
        factory(UserLedgerEntry::class)->create(
            ['user_id' => $user3->id, 'type' => UserLedgerEntry::TYPE_AD_INCOME, 'amount' => 100]
        );
        factory(UserLedgerEntry::class)->create(
            ['user_id' => $user3->id, 'type' => UserLedgerEntry::TYPE_BONUS_INCOME, 'amount' => 100]
        );
        factory(UserLedgerEntry::class)->create(
            ['user_id' => $user4->id, 'type' => UserLedgerEntry::TYPE_AD_INCOME, 'amount' => 500]
        );
