...
class PercentileComputer
{
    ...
    public function dumpToXlsx(string $category, int $occurrences): void
    {
        $title = 'title';
        $subject = 'subject';
        $description = 'description';
        $keywords = 'keywords';
        $spreadsheetCategory = 'Report';
        ...
        $spreadsheet->getProperties()
            ...
            ->setCategory($spreadsheetCategory);

        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle($spreadsheetCategory);

        $x = ord('A');
        $y = 1;

        $coordinate = chr($x).$y;
        $sheet->setCellValue($coordinate, 'Category');
        ++$x;
        $coordinate = chr($x).$y;
        $sheet->setCellValue($coordinate, 'Occurrences');
        --$x;
        ++$y;
        ...
        $coordinate = chr($x).$y;
        $sheet->setCellValue($coordinate, 'Campaign id');
        ++$x;
        $coordinate = chr($x).$y;
        $sheet->setCellValue($coordinate, 'Count');
        ++$x;
        $coordinate = chr($x).$y;
        $sheet->setCellValue($coordinate, 'Average');
        $x -= 2;
        ++$y;

        foreach ($this->campaigns as $campaignId => $campaignData) {
            $coordinate = chr($x).$y;
            $sheet->setCellValue($coordinate, $campaignId);
            ++$x;
            $coordinate = chr($x).$y;
            $sheet->setCellValue($coordinate, $campaignData[self::KEY_COUNT]);
            ++$x;
            $coordinate = chr($x).$y;
            $sheet->setCellValue($coordinate, $campaignData[self::KEY_AVERAGE]);
            $x -= 2;
            ++$y;
        }

        $writer = IOFactory::createWriter($spreadsheet, 'Xlsx');
        $writer->save(str_replace(':', '-', $category).'.xlsx');
    }
