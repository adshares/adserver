...
class SiteFilteringUpdater
{
    ...
    public function addClassificationToFiltering(Site $site): void
    {
        ...
        if(config('app.reviewer_user_id')){
            ...
            foreach ($requireKeywords as $requireKeyword) {
                $siteExcludes[self::INTERNAL_CLASSIFIER_NAMESPACE][] = $requireKeyword->keyword();
            }
        }
     ...
     * @return Classification[]
     */
    private function getClassificationForRejectedBannersForAllSites(int $publisherId): array
    {
        return [
