...
MYSCRIPT;
    ...
    private function replaceCssUrls($css_text, $basedir)
    {
        ...
        return preg_replace_callback('#url\(\s*[\'"]?([0-9a-z' . $uri_chars . ']+?)[\'"]?\s*\)#im', function($match) use($basedir) {
            $scheme = parse_url($match[1], PHP_URL_SCHEME);
            if($scheme) {
                if($scheme == 'data') {
                    return $match[0];
                } else {
                    throw new \RuntimeException(sprintf("Only local assets and data uri allowed (found %s)", $match[1]));
                }
            }
    ...
    private function loadFile()
    {
        ...
        if ($zipped_size >= self::MAX_ZIPPED_SIZE) {
            throw new \RuntimeException(sprintf("Zip file max size exceeded (%d KB > %d KB)  ", $zipped_size / 1024, self::MAX_ZIPPED_SIZE / 1024));
        }
        ...
        if ($zip) {
            while ($zip_entry = \zip_read($zip)) {
                ...
                if ($unzipped_size >= self::MAX_UNZIPPED_SIZE) {
                    throw new \RuntimeException(sprintf("Zip file max uncompressed size exceeded (%d KB > %d KB)", $unzipped_size / 1024, self::MAX_UNZIPPED_SIZE / 1024));
                }
